...
function saveFired(fired) {
    ...
    catch (e) { console.error('[WEEKLY] Erreur sauvegarde fired.json :', e); }
}

// Heure Paris (gere hiver/ete automatiquement)
...
function startWeeklyTask(client) {
    ...
    setInterval(() => {
        ...
        for (const task of tasks) {
            ...
            // Fenetre de declenchement : jusqu'a 59 min apres l'heure cible
            // (permet de rattraper le retard si Render s'est endormi)
            if (now.getHours() > task.hour) {
                // Heure deja depassee — on envoie quand meme sauf si c'est le lendemain
                // (getDay() est toujours SAT donc on est bon)
            }

            fired.add(key);
            saveFired(fired);
            console.log(`[WEEKLY] Declenchement ${task.hour}h (Paris: ${now.getHours()}h${now.getMinutes()})`);
            task.fn().catch(err => console.error(`[WEEKLY] Erreur tache ${task.hour}h :`, err));
        }
    }, 60_000); // toutes les minutes

    // Verifier immediatement au demarrage sans attendre 1 minute
    setTimeout(() => {
        const now = getNowParis();
        const weekKey = getWeekKey();
        console.log(`[WEEKLY] Demarrage — heure Paris: ${now.getHours()}h${now.getMinutes()} jour: ${now.getDay()}`);
        ...
        for (const task of tasks) {
            ...
            fired.add(key);
            saveFired(fired);
            console.log(`[WEEKLY] Rattrapage au demarrage — tache ${task.hour}h`);
            task.fn().catch(err => console.error(`[WEEKLY] Erreur rattrapage ${task.hour}h :`, err));
        }
    }, 5_000); // 5 secondes apres le demarrage

    console.log('[WEEKLY] Scheduler demarre');
}

...
.val.ok{color:#3fb950;}.val.ko{color:#f85149;}
...
<div class="summary-bar">
    ...
    <div class="summary-stat"><div class="val ok">${done}</div><div class="lbl">Complété</div></div>
    <div class="summary-stat"><div class="val ko">${miss}</div><div class="lbl">Manquant</div></div>
</div>
...
function startWeeklyTask(client) {
    ...
    function checkTasks() {
        ...
        for (const task of tasks) {
            ...
            fired.add(key);
            saveFired(fired);
            console.log(`[WEEKLY] Declenchement ${task.hour}h (Paris: ${now.getHours()}h${String(now.getMinutes()).padStart(2, '0')})`);
    ...
    const now = getNowParis();
    console.log(`[WEEKLY] Scheduler demarre — Paris: ${now.getHours()}h${String(now.getMinutes()).padStart(2, '0')} jour: ${now.getDay()}`);
}
